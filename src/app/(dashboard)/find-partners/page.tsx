      ...
      <div className="bg-surface-soft border-b border-hairline p-6 rounded-2xl shadow-elevation-1 space-y-4">
        ...
        <div className="relative w-full">
          ...
          <input
            ...
            className="w-full pl-11 pr-10 h-11 bg-canvas border border-hairline rounded-xl text-xs text-ink-deep placeholder:text-slate focus:outline-none focus:border-primary transition-colors shadow-sm"
          />
          {searchQuery && (
            <button
              ...
              <button
                ...
                className={`flex items-center gap-1.5 px-3.5 py-1.5 rounded-full text-xs font-semibold whitespace-nowrap transition-all ${
                  isActive
                    ...
                    : 'bg-canvas border border-hairline text-slate hover:border-primary/50 hover:text-ink-deep'
                }`}
              >
      ...
      <div className="space-y-4">
        {searchResults === undefined ? (
          ...
          <div className="text-center py-12 bg-surface-soft border border-hairline rounded-2xl p-6">
            <Users className="w-10 h-10 text-slate mx-auto mb-2" />
            <h3 className="text-sm font-semibold text-ink-deep">No partners found</h3>
          ...
          <div className="space-y-4">
            {users.map((user: any) => {
              ...
              return (
                <div
                  key={targetUserId}
                  className="group rounded-2xl border border-hairline bg-surface-soft p-5 hover:border-primary/40 hover:shadow-elevation-1 transition-all duration-200"
                >
                      ...
                      <AvatarWithStatus userId={targetUserId} size="md">
                        <div className="relative h-12 w-12 flex-shrink-0">
                          {user.profilePicture ? (
                            ...
                            <div className="flex h-12 w-12 items-center justify-center rounded-full bg-primary/10 text-primary font-bold text-base border border-hairline">
                    ...
                    <div className="flex flex-wrap gap-1.5 mt-4 pt-3 border-t border-hairline/60">
                      {user.skills.slice(0, 5).map((skill: string) => (
                        <span
                          ...
                          className="text-[11px] px-2.5 py-1 rounded-lg bg-canvas border border-hairline text-ink-deep hover:border-primary cursor-pointer transition-colors"
                        >
                          {skill}
                        </span>
                      ))}
                      {user.skills.length > 5 && (
                        <span className="text-[11px] text-slate font-medium self-center pl-1">
                  ...
                  <div className="mt-4 flex items-center gap-2">
                    <Link
                      ...
                      className="flex-1 flex items-center justify-center gap-1.5 py-2 px-3 rounded-xl border border-hairline bg-canvas text-xs font-semibold text-ink-deep hover:border-primary/50 transition-colors"
                    >
                    ...
                    <button
                      ...
                      className="py-2 px-4 rounded-xl bg-surface-soft border border-hairline text-xs font-semibold text-primary hover:bg-primary/10 hover:border-primary/30 transition-colors flex items-center gap-1.5"
                    >
