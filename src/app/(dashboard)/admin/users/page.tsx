      ...
      <div className="flex flex-col sm:flex-row items-center justify-between mb-8 gap-4">
        ...
        <div className="relative w-full sm:w-64">
          ...
          <input 
            ...
            className="w-full pl-9 pr-4 py-2 border rounded-full bg-background"
            value={searchTerm}
                ...
                <tr key={user.id} className="border-b last:border-0 hover:bg-muted/20">
                  ...
                  <td className="p-4">
                    <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${
                      ...
                      user.role === 'suspended' ? 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400' :
                  ...
                  <td className="p-4 text-right">
                    <div className="flex items-center justify-end gap-2">
                      {user.role !== 'admin' && (
                        <>
                          <button onClick={() => handleAction(user.id, "make_admin")} className="p-2 hover:bg-muted rounded-full text-primary" title="Make Admin">
                            <Shield className="w-4 h-4" />
                          </button>
                          {user.role === 'suspended' ? (
                            <button onClick={() => handleAction(user.id, "restore")} className="p-2 hover:bg-muted rounded-full text-green-600" title="Restore User">
                              <CheckCircle className="w-4 h-4" />
                            </button>
                          ) : (
                            <button onClick={() => handleAction(user.id, "suspend")} className="p-2 hover:bg-muted rounded-full text-red-500" title="Suspend User">
                      ...
                      {user.role === 'admin' && user.id !== currentUser._id && (
                        <button onClick={() => handleAction(user.id, "restore")} className="p-2 hover:bg-muted rounded-full text-red-500" title="Remove Admin">
                          <Ban className="w-4 h-4" />
                        </button>
                      )}
                    </div>
