...
function StoryViewerContent({ storyId }: { storyId: Id<'stories'> }) {
    ...
    }, [storyId, isAuthenticated, isVideo, story, startProgressBar]);
        ...
        <div className="fixed inset-0 z-50 flex flex-col items-center justify-center bg-black select-none">
            ...
            {isOwnStory && (
                <div className="absolute bottom-6 left-0 right-0 z-20 flex justify-center gap-6 text-white">
                    <div className="flex items-center gap-1.5 text-sm bg-black/40 px-3 py-1.5 rounded-full backdrop-blur-md">
                        <Eye className="h-4 w-4" /> {story.viewCount} Views
                    </div>
                    <button onClick={handleDelete} className="flex items-center gap-1.5 text-sm hover:text-red-400 bg-black/40 px-3 py-1.5 rounded-full backdrop-blur-md transition-colors">
