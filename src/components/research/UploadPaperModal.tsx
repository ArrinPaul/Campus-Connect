...
export function UploadPaperModal({ onClose }: UploadPaperModalProps) {
 ...
 >
 {author}
 <button onClick={() => removeAuthor(author)} className="hover:text-critical">
 <X className="h-3 w-3" />
 </button>
 </span>
 ))}
 ...
 >
 #{tag}
 <button onClick={() => removeTag(tag)} className="hover:text-critical">
 <X className="h-3 w-3" />
 </button>
 </span>
 ))}
 ...
 className="px-4 py-2 text-sm font-semibold bg-primary text-on-primary rounded-lg hover:bg-primary/90 disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
 >
 {isSubmitting ? (
 <>
