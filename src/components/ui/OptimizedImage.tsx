...
/**
 * Checks if a string is a valid web image source (relative URL, http/https, blob, or data URL).
 */
function isValidWebSrc(src: any): boolean {
  if (typeof src !== 'string') return true; // StaticImageData object is valid
  if (!src) return false;
  // Local filesystem Windows paths (e.g. C:\... or file:///) are not web loadable
  if (/^[a-zA-Z]:\\|^file:\/\//i.test(src)) return false;
 ...
 * lazy loading, error fallback handling, and local path protection.
 */
export function OptimizedImage({
  ...
  const isValidSrc = useMemo(() => isValidWebSrc(src), [src]);

  if (hasError || !isValidSrc) {
    if (fallback) return <>{fallback}</>;
    return (
      <div
        className={`bg-zinc-800/80 flex items-center justify-center text-zinc-400 ${className ?? ''}`}
