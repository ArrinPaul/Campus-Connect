...
const statusOptions: { value: Status; label: string; icon: React.ReactNode; color: string; description: string }[] = [
 ...
 icon: <MinusCircle className="h-3 w-3 fill-red-500 text-critical" />,
 color:"text-critical",
...
export function StatusSelector({
 ...
 maxLength={100}
 className="flex-1 rounded-lg border border-hairline bg-card bg-muted px-3 py-2 text-sm text-ink-deep placeholder:text-muted-foreground dark:placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary"
 />
 {customStatus !== currentCustomStatus && (
 <button
 onClick={handleCustomStatusSave}
 disabled={isUpdating}
 className="px-3 py-2 text-sm font-medium text-on-primary bg-primary hover:bg-primary/90 rounded-lg disabled:opacity-50"
 >
 Save
 </button>
 )}
