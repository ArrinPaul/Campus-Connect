...
export function CodeBlock({ code, language ="text", filename, className }: CodeBlockProps) {
 ...
 return (
 <div className={cn("group relative my-3 rounded-xl overflow-hidden border border-hairline bg-[#1e1e2e]", className)}>
 {/* Header */}
 ...
 {filename && (
 <span className="ml-2 text-xs text-on-primary/50 font-mono">{filename}</span>
 )}
 </div>
 <div className="flex items-center gap-2">
 <span className="text-xs text-on-primary/30 font-mono uppercase tracking-wide">{normalizedLang}</span>
 <button
 onClick={handleCopy}
 className="rounded-md p-1 text-on-primary/40 hover:text-on-primary/80 hover:bg-card/10 transition-colors"
