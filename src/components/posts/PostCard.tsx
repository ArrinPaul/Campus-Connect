...
import { Share2, Copy, Repeat2, MessageCircle, Trash2, MoreHorizontal, Pencil, Check, X } from "lucide-react"
...
export const PostCard = memo(function PostCard({ post, author }: PostCardProps) {
  ...
  const [updatedAtTimestamp, setUpdatedAtTimestamp] = useState<number | string | undefined>(
 ...
 :"skip"
  ...
  useEffect(() => {
    if (commentsData && !isLoadingMoreComments) {
      setAllComments(Array.isArray(commentsData.comments) ? commentsData.comments : [])
  ...
  useEffect(() => {
    if (moreCommentsData && isLoadingMoreComments) {
      setAllComments((prev) => {
        ...
        const newCommentsList = Array.isArray(moreCommentsData.comments) ? moreCommentsData.comments : []
        const existingIds = new Set(safePrev.map((c: any) => c?._id || c?.id))
        const newComments = newCommentsList.filter((c: any) => !existingIds.has(c?._id || c?.id))
 ...
 return () => document.removeEventListener("mousedown", handleClickOutside)
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      await updatePostMutation({ postId: post._id, content: editContent.trim() })
...
"Research Scholar": {
 ...
 <p className="font-semibold text-ink-deep truncate hover:underline cursor-pointer">{author.name}</p>
 <span className={cn(
"inline-flex items-center rounded-full border px-1.5 py-0 text-[10px] font-semibold uppercase tracking-wide",
 ...
 <Link href={`/post/${post._id}`} className="hover:underline flex items-center gap-1">
 <p className="text-xs text-slate">{formatTimestamp(post.createdAt)}</p>
                  {Boolean(updatedAtTimestamp) && formatTimestamp(updatedAtTimestamp) !== "" && (
                    <span className="text-[11px] text-slate/70 italic">
 ...
 <p className="text-[10px] text-slate uppercase font-semibold">Confirm Delete?</p>
...
"active:scale-[0.98] flex items-center gap-1.5 rounded-full px-3 py-1.5 text-slate transition-colors hover:bg-canvas hover:text-primary",
 ...
 <span className="text-xs text-slate font-semibold">{post.commentCount}</span>
 </motion.button>
 ...
 {post.shareCount > 0 && (
 <span className="text-xs text-slate font-semibold">{post.shareCount}</span>
 )}
 </motion.button>
 ...
 <CommentList
 postId={post._id}
 comments={(Array.isArray(allComments) && allComments.length > 0) ? allComments : (Array.isArray(commentsData?.comments) ? commentsData.comments : [])}
