...
export function CommentList({
 ...
 className="text-xs text-critical hover:text-critical/80 disabled:opacity-50"
 >
 ...
 />
 {replyError && (
 <p className="text-xs text-critical">{replyError}</p>
 )}
 <div className="flex justify-end">
 <button
 onClick={() => handleReplySubmit(comment._id)}
 disabled={isSubmittingReply || !replyContent.trim()}
 className="rounded-md bg-primary px-3 py-1.5 text-xs text-on-primary hover:bg-primary/90 disabled:opacity-50 flex items-center gap-1"
 >
