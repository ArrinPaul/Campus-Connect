...
export function TrendingHashtags({ limit = 5, className = "" }: TrendingHashtagsProps) {
  ...
  return (
    <div className={`rounded-lg border border-border bg-card p-4 shadow-sm ${className}`} data-testid="trending-hashtags-widget">
      <div className="flex items-center gap-2 pb-3 border-b border-border mb-3">
        <TrendingUp className="h-5 w-5 text-primary" />
        <h3 className="font-semibold text-foreground text-sm">Trending on Campus</h3>
      </div>
        ...
        <div className="space-y-2" data-testid="trending-loading">
          {[...Array(3)].map((_, i) => (
            <div key={i} className="animate-pulse flex items-center justify-between py-1.5">
              ...
              <div className="h-3 bg-card rounded w-12"></div>
            </div>
          ))}
        </div>
      ) : hashtags.length === 0 ? (
        ...
        <div className="space-y-1.5" data-testid="trending-list">
          {hashtags.map((item) => (
            <Link
              ...
              className="flex items-center justify-between rounded-lg px-2 py-1.5 hover:bg-card transition-colors group"
            >
